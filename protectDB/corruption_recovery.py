...
#partitionSz = 2* leaves_per_partition -1
...
# print("unknown dict key = " + func_key['rand'])
...
def convert(data):
        ...
        for item in data:
          #print(item)
          #type(item) is int:
          #  noDecimaldata.append(item)
          #if type(item) is Decimal:
          if isinstance(item, decimal.Decimal):
            #noDecimaldata.append(tuple(str(item)))
            #noDecimaldata += .append(float(item))
...
def tupsetToList(rows1, rows2):
     #toDelete = list(set(rows1).difference(rows2)) 

     keylist = ''
     for j in range(len(rows1)):
        set1 = {r[0] for r in rows1[j]}
        set2 = {r[0] for r in rows2[j]}
        print('set1 = ' + str(rows1))
        print('set2 = ' + str(rows2))
    # 1. Find the difference and convert to a sorted list
        datadiff = sorted(set1 -  set2)
        print('sdiff = ' + str(datadiff))
        raw_str = datadiff[0]

    # 2. Remove the curly braces and split by comma
    # We use .strip("{}") to remove the brackets at the ends
        clean_klist = raw_str.strip("{}").split(",")

    # 3. Convert each string piece into an actual integer
        nkey = 0
        for key in clean_klist:
            keylist += str(key) 
            nkey += 1
...
try:
    ...
    while (is_leaf is False):
      ...
      #nodes = { k: rows1[k] for k in rows1 if k in rows2 and rows1[k] == rows2[k]}
      #nodes = set(rows1.items()) ^ set(rows2.items())
      #print(rows1)
    ...
    print("\n  merkle leaf IDs= " + leafids)
      ...
      #treeid = int((int(node) -1) / partitionSz )
      #nodeSubId = int(node) - treeid * partitionSz - leaves_per_partition
      #leafid = (treeid ) * leaves_per_partition + int(nodeSubId) - 1
    ...
    keylist += (getkeys(rows2))
      #clean_klist, keylist = tupsetToList(rows1[0], rows2[0])
      #clean_klist, keylist = tupsetToList(rows1[1], rows2[1])
    keys = keylist.split(',')
    kset = (set(keys))
    ...
    detect_time = time.time_ns()
      #print('\nStep (2c) Differing record = ' + str(item[0]) + '\n')
    ...
    if(DBG == 1):
        for row in rows1:
          print('row=== ' + str(row[0]))
    if(len(keylist) != 0):
        #print("del list = " + ''.join(map(str,toDelete)))
        #print(toDelete)
      ...
      #addupd = list(set(rows2).difference(rows1))  # id = 3upd, 2 add
      #clean_klist, keylist = tupsetToList((rows2), (rows1))
    #print('klist = ' + str(keylist))
    ...
    if(DBG == 1):
      ...
      #for item in addupd:
        #print('\n Differing row = ' + str(item) + '\n')
      #print("addupd len = " + str(len(addupd)))
    if(len(rows2) == 0):
        print("add list null ")
    else:
       #print("addupd len== " + str(len(addupd)) + " == ", end=" ")
       #print(' commands = ')
       for data in rows2:
          query = convert(data)
