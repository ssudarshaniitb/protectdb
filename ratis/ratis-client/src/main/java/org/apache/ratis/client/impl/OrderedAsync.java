...
public final class OrderedAsync {
  ...
  private void sendRequestWithRetry(PendingOrderedRequest pending) {
    LOG.debug( "safedb clazz=" + this.getClass().getSimpleName() +"  func=" + Thread.currentThread().getStackTrace()[1].getMethodName());
    if(dbg) System.err.println( "safedb syserr  clazz=" + this.getClass().getSimpleName() +"  func="  + Thread.currentThread().getStackTrace()[1].getMethodName());
