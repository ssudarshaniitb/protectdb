...
public class FileStore implements Closeable {
  ...
  CompletableFuture<WriteReplyProto> submitCommitDb(
    long index, String relative, boolean close, long offset, int size, String replyString) {
  ...
  return uc.submitCommit(offset, size, converter, committer, getId(), index)
      .thenApply(n -> WriteReplyProto.newBuilder()
          .setResolvedPath(FileStoreCommon.toByteString(replyString))
          .setOffset(offset)
          .setLength(n)
          .build());
}
