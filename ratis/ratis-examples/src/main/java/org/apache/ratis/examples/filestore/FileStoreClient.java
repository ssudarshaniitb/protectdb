...
public class FileStoreClient implements Closeable {
  ...
  public long write(String path, long offset, boolean close, ByteBuffer buffer, boolean sync)
    ...
    LOG.debug( "safedb data=" + reply.toString());
    if(dbg) System.err.println( "safedb syserr  data=" + reply.toString());
  ...
  public CompletableFuture<Long> writeAsync(String path, long offset, boolean close, ByteBuffer buffer, boolean sync) {
    ...
    if(dbg) System.err.println( "safedb syserr  clazz=" + this.getClass().getName() +"  func="  + Thread.currentThread().getStackTrace()[1].getMethodName());
      ...
      LOG.debug( "safedb data=" + buffer.toString() + "  " + ByteString.copyFrom(buffer).toString());
      if(dbg) 
        System.err.println( "safedb writeAsync syserr  data=" + buffer.toString() + " &&& " + ByteString.copyFrom(buffer).toString());
  ...
  private static <OUTPUT, THROWABLE extends Throwable> OUTPUT writeImpl(
      ...
      throws THROWABLE {
        LOG.debug( "safedb " +  "  func=" + Thread.currentThread().getStackTrace()[1].getMethodName());
        if(dbg) System.err.println( "safedb syserr " +  "  func="  + Thread.currentThread().getStackTrace()[1].getMethodName());
        LOG.debug( "safedb data=" + data.toString() + " &&& " + ByteString.copyFrom(data).toString());
        data.flip();
      //if(dbg) 
        System.out.println( "safedb syserr  data=" + data.toString() + " &&& " + ByteString.copyFrom(data).toString());
      data.flip();
