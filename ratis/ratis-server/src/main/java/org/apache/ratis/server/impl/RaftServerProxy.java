...
class RaftServerProxy implements RaftServer {
  ...
  RaftServerProxy(RaftPeerId id, StateMachine.Registry stateMachineRegistry,
    ...
    RaftServerConfigKeys.Log.setAsyncFlushEnabled(this.properties, true);
RaftServerConfigKeys.Log.setWriteBufferSize(this.properties,
        SizeInBytes.valueOf("64MB"));
/*RaftServerConfigKeys.Log.setFlushBatchSize(this.properties,
        SizeInBytes.valueOf("8MB"));
RaftServerConfigKeys.Log.flush.setFlushInterval(this.properties,
        TimeDuration.valueOf(10, TimeUnit.MILLISECONDS));
*/
// segment size
RaftServerConfigKeys.Log.setSegmentSizeMax(this.properties,
        SizeInBytes.valueOf("256MB"));
