 ...
 * block.  It will point to TopTransactionStateDta when not in a
...
GetCurrentTransactionId(void)
{
	...
	if (!FullTransactionIdIsValid(s->fullTransactionId))
	{
		...
		if (activeTx)
		{
	        activeTx->xid = XidFromFullTransactionId(s->fullTransactionId);
	        add_tx_xid_map(activeTx->xid, activeTx);
		}
	}
...
AbortTransaction(void)
{
	...
	if (TopTransactionResourceOwner != NULL)
	{
		...
		//if (is_bcdb_worker)
		//	LWLockAcquire(&activeTx->lock, LW_EXCLUSIVE);
		ResourceOwnerRelease(TopTransactionResourceOwner,
							 RESOURCE_RELEASE_LOCKS,
							 false, true);
		ResourceOwnerRelease(TopTransactionResourceOwner,
...
void
SetCurrentTransactionState(TransactionState tx_state){
    CurrentTransactionState = tx_state;
}

void
AttachTopTransactionState(TransactionState tx_state){
    TopTransactionStateData = *tx_state;
}

TransactionState
DetachTopTransactionState(void)
{
    TransactionState ret;

    ret = MemoryContextAlloc(MessageContext, sizeof(TransactionStateData));
    *ret = TopTransactionStateData;

    return ret;
}

//todo: some key xact variable may be stuck....
void
ResetTopTransactionState(void)
{
    TransactionState s;
