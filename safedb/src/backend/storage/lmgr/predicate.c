...
CheckAndPromotePredicateLockRequest(const PREDICATELOCKTARGETTAG *reqtag)
{
	...
	/* don't do promotion because it will cause non-determinism */
	if (is_bcdb_worker)
		return false;
					...
					SERIALIZABLEXACT *sxact)
{
	...
	if (!target)
		ereport(ERROR,
				(errcode(ERRCODE_OUT_OF_MEMORY),
				 errmsg("out of shared memory in predlock target"),
	...
	if (!lock)
		ereport(ERROR,
				(errcode(ERRCODE_OUT_OF_MEMORY),
				 errmsg("out of shared memory in predlock"),
				 errhint("You might need to increase max_pred_locks_per_transaction.")));
	...
	while (predlock)
	{
		...
		if (summarize)
		{
			...
			if (!predlock)
				ereport(ERROR,
						(errcode(ERRCODE_OUT_OF_MEMORY),
						 errmsg("out of shared memory in sumarizing predlock"),
						 errhint("You might need to increase max_pred_locks_per_transaction.")));
			if (found)
			{
				Assert(predlock->commitSeqNo != 0);
								...
								Snapshot snapshot)
{
	...
	if (SxactIsDoomed(MySerializableXact))
	{
		if (is_bcdb_worker)
			ereport(ERROR,
					(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
					 errmsg("tx %s doomed, because %s", activeTx->hash, activeTx->why_doomed)));
		else
			ereport(ERROR,
					(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
	...
	/* the potentially conflicting transction must be cocurrent with us now */

	Assert(TransactionIdIsValid(xid));

	if (!is_bcdb_worker)
	{
		/* no need for this safety check since we are concurrent with xid */
	...
	if (!is_bcdb_worker && TransactionIdPrecedes(xid, TransactionXmin))
		return false;
	if (TransactionIdEquals(xid, GetTopTransactionIdIfAny()))
		return false;
	...
	if (!is_bcdb_worker && !XidIsConcurrent(xid))
	{
