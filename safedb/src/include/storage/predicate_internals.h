...
typedef struct SERIALIZABLEXACT
{
	...
	int			  pre_ts; /* visit timestamp: for conflict resolution */
	int			  post_ts; /* leave timestamp: for conflict resolution */
	/* it is actually BCDBShmXact type, but we want to avoid circular references */
	void	     *bcdb_tx;
} SERIALIZABLEXACT;
...
#define PredicateLockTagIsRelationLock(locktag) ((locktag).locktag_field3 == InvalidBlockNumber && (locktag).locktag_field4 == InvalidOffsetNumber)
#define PredicateLockTagIsPageLock(locktag) ((locktag).locktag_field3 != InvalidBlockNumber && (locktag).locktag_field4 == InvalidOffsetNumber)
#define PredicateLockTagIsTupleLock(locktag) ((locktag).locktag_field3 != InvalidBlockNumber && (locktag).locktag_field4 != InvalidOffsetNumber)
...
extern void RemoveTargetIfNoLongerUsed(PREDICATELOCKTARGET *target,
...
extern HTAB *SerializableXidHash;
