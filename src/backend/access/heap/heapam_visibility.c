...
static bool BCDBHeapTupleSatisfiesBlock(Snapshot snapshot, HeapTuple tup, Buffer buffer);
						 ...
						 Buffer buffer)
{
	...
	if (HeapTupleHeaderIsBeingModified(tuple))
		return TM_BeingModified;
...
bool BCDBHeapTupleSatisfiesBlock(Snapshot snapshot, HeapTuple tup, Buffer buffer){
    ...
    current_bid = GetCurrentTxBlockIdSnapshot();
    	...
    	//	printf("ariaMyDbg %s : %s: %d BCDB bmn %d bmx %d cubd %d \n",
	//		 __FILE__, __FUNCTION__, __LINE__, bcdb_bmin, bcdb_bmax, current_bid);
