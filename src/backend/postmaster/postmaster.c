...
PostmasterMain(int argc, char *argv[])
{
	...
	/*
		 ...
		 * since there is no way to connect to the database in
	...
	/* create dynamic shared memory in postmaster before accepting connections */
...
retry1:
	...
	if (PG_PROTOCOL_MAJOR(proto) >= 3)
	{
		...
		while (offset < len)
		{
			...
			else if (strcmp(nameptr, "host") == 0)
				bcdb_host = pstrdup(valptr);
			else if (strcmp(nameptr, "port") == 0)
				bcdb_port = pstrdup(valptr);
